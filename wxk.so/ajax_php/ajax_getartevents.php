...
$strSQL = "SELECT a.*,b.name as newsdirname,c.name as username,c.photo FROM newsinfo as a
		   ...
		   where  find_in_set('10',a.dirtree)  and c.photo!='' and a.id_member=$c_memberid
...
$arr[info].='<div class="messagebox"><div class="innercontent"><div>'.$allNewsInfo[$i][title].'</div><div class="innercontentimgs"><ul id="EventsGallery'.($start+$i).'" class="gallery">'.$strpic.'</ul><div style="clear:both"></div></div><div style="clear:both">'.$allNewsInfo[$i][newsdate].'</div></div><div style="clear:both"></div></div>';
...
if(sizeof($artzuopinpic)!=0){
	if($i==$allNewsInfoNum-1){
	   $arr[picshowid].='#EventsGallery'.($start+$i)." a";
	}else{
	   $arr[picshowid].='#EventsGallery'.($start+$i)." a,";	
	}
}
