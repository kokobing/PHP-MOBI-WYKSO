...
if (!empty($_FILES) && $_POST['token'] == $verifyToken) {
	...
	if (in_array(strtolower($fileParts['extension']), $fileTypes)) {
		...
		$strSQL="INSERT INTO eventpic(id_eventinfo,id_member,opicname,indate) values ('".$_POST[insertid]."','".$_SESSION[memberid]."','$new_file_name',now())";
		$objDB->Execute($strSQL);
	
//file_put_contents('aa.txt',$strSQL); 

		$ordernumid=$objDB->getInsertID();//获取插入到数据库记录的ID号
		$strSQL="UPDATE eventpic SET ordernum='$ordernumid'   where id_eventinfo='$ordernumid'";
		$objDB->Execute($strSQL);
