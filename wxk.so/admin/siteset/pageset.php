<?php
require "../inc/core/site/pageset_core.php";
...
<head>
    ...
    <link href="/<?=SITE_DIR?>/inc/css/jquery.fileupload.css" rel="stylesheet" type="text/css">
    <link href="/<?=SITE_DIR?>/inc/css/jquery.fileupload-ui.css" rel="stylesheet" type="text/css">
    <link href="/<?=SITE_DIR?>/inc/css/bootstrap-wysihtml5-0.0.2.css" rel="stylesheet" type="text/css">  
              ...
              <div class="col-md-12">
                <div class="panel widget">
                  <div class="panel-heading vd_bg-grey">
                    <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-<?=$leftmenu_icon;?>"></i> </span> 版面管理
                    
                    <div class="menu-icon" style="float:right; position:relative"> 
                         <a href="/<?=SITE_DIR?>/siteset/pageset-add.html" class="custom_add"><i class="fa fa-plus"></i></a>
                    </div>
                  ...
                  <div class="panel-body  table-responsive">
                    <table class="table table-striped table-hover">
                      ...
                      <?php for($i=0;$i<$PagelistNum;$i++){?>
                        <tr>
                          <td><?=$i+1;?></td>
                          <td><?=$Pagelist[$i][title]?></td>
                          <td><?=cutstr($Pagelist[$i][intro],70,1)?></td>
                          <td><?php if($Pagelist[$i][id_lang]=='1'){echo '<span class="label vd_bg-red" >中文</span>';}elseif($Pagelist[$i][id_lang]=='2'){echo '<span class="label vd_bg-blue">英文</span';}?></td>
                          <td>
                          
                          <? if($ispermit[ED]){//有编辑权?>
                          
                             <? if($Pagelist[$i][status]=='1'){ ?>
                                 <a href="javascript:void(0)" onClick="Mstatus('<?=$Pagelist[$i][id_pageset]?>','<?=$Pagelist[$i][status]?>');"><span class="label label-success">开通</span></a>
                             <? }else{?>
                                 <a href="javascript:void(0)" onClick="Mstatus('<?=$Pagelist[$i][id_pageset]?>','<?=$Pagelist[$i][status]?>');"><span class="label label-danger">暂停</span></a>
                              <? }?>   
                          ...
                          <td class="menu-action rounded-btn">
                              <? if( $ispermit[ED]){?><a href="pageseteditlist-<?=$Pagelist[$i][id_pageset];?>.html" data-original-title="编辑" data-toggle="tooltip" data-placement="top" class="btn custom-table-icon vd_bg-yellow"> <i class="fa fa-pencil"></i> </a> <? }?>
                              <? if( $ispermit[DE]){?><a  href='javascript:void(0)' onclick='pagedel(<?=$Pagelist[$i][id_pageset];?>);' data-original-title="删除" data-toggle="tooltip" data-placement="top" class="btn custom-table-icon vd_bg-red"> <i class="fa fa-times"></i> </a><? }?>
                          </td>
                        </tr>
                      <?php }?>
...
<div class="vd_content-section clearfix">
          ...
          <div class="row">
              <div class="col-md-12">
                ...
                <div class="panel widget">
                  <div class="panel-heading vd_bg-grey">
                    <h3 class="panel-title"> <span class="menu-icon"> <i class="fa fa-<?=$leftmenu_icon;?>"></i> </span> 编辑 <?=$SetPagesetinfo[title]?> 版面 <div style=" float:right; margin:0;text-align:right"> <a href="javascript:void(0)" onClick="history.go(-1);" class="btn btn-primary btn-xs">返回上一层</a> </div> </h3>
                  </div>
                  <div class="panel-body">
                    <form class="form-horizontal" action="pagesetedit-<?=$SetPagesetinfo[id_pageset]?>.html" role="form"  method="post">
                     
                      <div class="form-group">
                        <label class="col-sm-2 control-label">版面名称</label>
                        <div class="col-sm-5 controls">
                          <input name="title" id="title" type="text" placeholder="请输入版面名称"  value="<?=$SetPagesetinfo[title]?>">
                          <span class="help-inline">版面名称将在列表页面显示</span> </div>
                      </div>
                        ...
                        <div class="col-sm-7 controls">
                                  <select  class="width-40"   id="catalogdirlang" name="catalogdirlang" placeholder="语言类别" >
                                     <? for($i=0;$i<$LangListNum;$i++){?>
                                      <option value="<?=$LangList[$i][id_lang]?>" <? if($SetPagesetinfo[id_lang]==$LangList[$i][id_lang]){echo 'selected';}?>><?=$LangList[$i][lang]?></option>
                                      <? }?>
                                  </select>
                        </div>
                      </div>
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-sm-5 controls">
                          <input name="pagetitle" id="pagetitle" type="text" placeholder="请输入页面标题" value="<?=$SetPagesetinfo[pagetitle]?>">
                          <span class="help-inline">页面标题将显示到header的title里</span> </div>
                      </div>

                      <div class="form-group">
                        <label class="col-sm-2 control-label">页面关健词</label>
                        <div class="col-sm-5 controls">
                          <input name="keywords" id="keywords" type="text" placeholder="请输入页面关健词" value="<?=$SetPagesetinfo[keywords]?>">
                          <span class="help-inline">页面标题将显示到header的keywords里</span> </div>
                      </div>

                      <div class="form-group">
                        <label class="col-sm-2 control-label">页面描述</label>
                        <div class="col-sm-7 controls">
                          <textarea name="description" id="description" rows="3" placeholder="请输入页面描述"><?=$SetPagesetinfo[description]?></textarea>
                          <span class="help-inline">页面标题将显示到header的description里</span> </div>
                      </div>

                      <div class="form-group">
                        <label class="col-sm-2 control-label">版面简介</label>
                        <div class="col-sm-7 controls">
                          <textarea name="pageintro" id="pageintro" rows="3" placeholder="请输入版面简介"><?=$SetPagesetinfo[intro]?></textarea>
                          <span class="help-inline">版面名称将在列表页面显示</span> </div>
                      </div>

                      <div class="form-group">
                        <label class="col-sm-2 control-label">版面备注</label>
                        <div class="col-sm-7 controls">
                          <textarea name="remark" id="remark" rows="3" placeholder="请输入版面备注"><?=$SetPagesetinfo[remark]?></textarea>
                          <span class="help-inline">版面备注是预留字段</span> </div>
                      </div>
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-sm-7 controls">
                          <textarea name="content" id="content" data-rel="ckeditor" rows="3"><?=$SetPagesetinfo[content]?></textarea>
                      ...
                      <div class="form-group">
                        ...
                        <div class="col-sm-7 controls">
                            ...
                            <span class="btn vd_btn vd_bg-green fileinput-button"> <i class="glyphicon glyphicon-plus"></i> 
                                <span>文件添加</span> 
                    ...
                    <div class="content-grid column-xs-1 column-sm-4 column-md-5 column-lg-6  height-xs-3">
                      <ul class="list-wrapper">
                      <?php 
						//抽取编辑版面名称
						$strSQL="SELECT * FROM pagesetpic WHERE  id_pageset='$_GET[editlist]' ";//需要编辑的版面
						$objDB->Execute($strSQL);
						$Pagesetinfopics=$objDB->getrows();
						$Pagesetinfopicsnum=sizeof($Pagesetinfopics);
						for($i=0;$i<$Pagesetinfopicsnum;$i++){
					  ?>
                        <li> <a href="#">
                          <div class="menu-icon"><img src="/upload/layout/<?=$Pagesetinfopics[$i][opicname]?>" alt="<?=$Pagesetinfopics[$i][title]?>"></div>
                          </a>
                          <div class="menu-text"><?=$Pagesetinfopics[$i][title]?>
                            <div class="menu-info">
                              <div class="menu-action"> <span data-placement="bottom" data-toggle="tooltip" data-original-title="编辑" class="menu-action-icon vd_green vd_bd-green"> <i class="fa fa-check"></i> </span> <span data-placement="bottom" data-toggle="tooltip" data-original-title="删除" class="menu-action-icon vd_red vd_bd-red"> <i class="fa fa-times"></i> </span> </div>
                            </div>
                          </div>
                        </li>
                      <?php }?>  
...
<? if($ispermit[ED]=='1'){?>
function Mstatus(id_pageset,status){
$.post('../ajax_php/site/ajax_setstatus.php',{id_pageset:id_pageset,status:status},function() {
            ...
            title: '版面删除提醒！',
             ...
             confirm: {confirm: true},
              buttons: {closer: false,sticker: false}
				...
				$.post('../ajax_php/site/ajax_delpage.php',{pageid:pageid},function() {
					window.location.reload();		
				});
