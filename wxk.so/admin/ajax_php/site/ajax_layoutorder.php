...
if( isset($_SESSION[username]) && $ajaxispermit[ED]){// 只有登陆用户才可以 并且拥有编辑权 
 ...
 //查找所有区块
	 $strSQL="SELECT * FROM layout WHERE 1 order by ordernum desc";
   ...
   if($_POST["arrow"]=='up'){//向上移动
	 ...
	 for($i=0;$i<$intnum;$i++){
		 ...
		 if(($_POST[layoutid]==$orderdir[$i][id_layout]) && $i!=0){//判断一级目录是否位置在最前，如果不到顶部就可以向上移动
			 
	             //             $orderdir[$i][id_layout] 当前ID      $orderdir[$i][ordernum]  当前顺序
				 //             $orderdir[$i-1][id_layout] 交换ID      $orderdir[$i-1][ordernum]  交换顺序
				 
				 $strSQL="UPDATE layout SET ordernum=". $orderdir[$i-1][ordernum]." WHERE id_layout=".$orderdir[$i][id_layout]."";
				 $objDB->Execute($strSQL);
				 $strSQL="UPDATE layout SET ordernum=".$orderdir[$i][ordernum]." WHERE id_layout=".$orderdir[$i-1][id_layout]."";	
				 $objDB->Execute($strSQL);
  ...
  if($_POST["arrow"]=='down'){//向下移动
	 ...
	 for($i=0;$i<$intnum;$i++){
		...
		if(($_POST[layoutid]==$orderdir[$i][id_layout]) && $i!=$intnum-1){//判断一级目录是否位置在最后，如果不在最后就可以向下移动
