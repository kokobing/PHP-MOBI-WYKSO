...
if( isset($_SESSION[username])){// 只有登陆用户才可以 
 ...
 if($ajaxispermit[ED]){
   ...
   if($_POST["status"]=='1'){//状态为1 则置0
     $strSQL="UPDATE pageset SET status='0' where  id_pageset='".$_POST[id_pageset]."' ";
   }else{
	 $strSQL="UPDATE pageset SET status='1' where  id_pageset='".$_POST[id_pageset]."' "; 
   }
