...
ajax_upload_pageset.php   版面管理 上传图片
...
if (!empty($_FILES) && $_POST['token'] == $verifyToken) {//如果上传不为空 并且令牌相等
	...
	if (in_array(strtolower($fileParts['extension']), $fileTypes)) {//扩展名正确匹配
		...
		//文件名入库
	     $strSQL="INSERT INTO pagesetpic(id_pageset,opicname) values ('".$_POST[Tableid]."','".$new_file_name."')";
		 $objDB->Execute($strSQL);
		 
		 $pid = $objDB->getInsertID();//获取刚入库记录的ID
         $strSQL="update pagesetpic set ordernum='".$pid."' where id_pagesetpic='".$pid."'";
