...
if( isset($_SESSION[username]) && $ajaxispermit[DE]){// 只有登陆用户才可以删除 
	   ...
	   //抽取需要删除图片的图片文件名
	   $strSQL="SELECT opicname FROM layoutpic WHERE id_layoutpic='".$_POST[picid]."' ";
	   $objDB->Execute($strSQL);
	   $onelayoutpic=$objDB->fields();
		
	  $strSQL="DELETE FROM layoutpic WHERE id_layoutpic='".$_POST[picid]."' ";//删除版面
      $objDB->Execute($strSQL);
	  
	@unlink("../../../upload/layout/".$onelayoutpic[opicname]);
