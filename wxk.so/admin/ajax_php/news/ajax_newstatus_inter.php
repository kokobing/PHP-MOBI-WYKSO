...
ajax_newstatus_inter.php    信息状态设置
...
if( isset($_SESSION[username])){// 只有登陆用户才可以 
 ...
 if($ajaxispermit[ED]){
   ...
   if($_POST["status"]=='1'){//状态为1 则置0
     $strSQL="UPDATE newsinfo SET status='0' where  id_newsinfo='".$_POST[newsid]."' ";
   }else{
	 $strSQL="UPDATE newsinfo SET status='1' where  id_newsinfo='".$_POST[newsid]."' "; 
   }
