...
ajax_upload_internalnews.php   新闻 上传图片
...
if (!empty($_FILES) && $_POST['token'] == $verifyToken) {//如果上传不为空 并且令牌相等
	...
	if (in_array(strtolower($fileParts['extension']), $fileTypes)) {//扩展名正确匹配
		...
		//取出图片关联的信息的添加人ID
		   ...
		   $strSQL="SELECT id_hr FROM newsinfo WHERE id_newsinfo='".$_POST[Tableid]."' ";	
		   $objDB->Execute($strSQL);
		   $onestaff=$objDB->fields();
		
		
		//文件名入库
	     $strSQL="INSERT INTO newspic(id_newsinfo,opicname,id_hr) values ('".$_POST[Tableid]."','".$new_file_name."','".$onestaff[id_hr]."')";
		 $objDB->Execute($strSQL);
		 
		 $pid = $objDB->getInsertID();//获取刚入库记录的ID
         $strSQL="update newspic set ordernum='".$pid."' where id_newspic='".$pid."'";
