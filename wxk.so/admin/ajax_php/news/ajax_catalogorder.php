...
if( isset($_SESSION[username]) && $ajaxispermit[ED]){// 只有登陆用户才可以 并且拥有编辑权 
 ...
 //查找同级目录
	 $strSQL="SELECT * FROM newsdir WHERE  fatherid=(select fatherid from newsdir where id_newsdir='".$_POST[dirid]."') order by ordernum desc";
   ...
   if($_POST["arrow"]=='up'){//向上移动
	 ...
	 for($i=0;$i<$intnum;$i++){
		 ...
		 if(($_POST[dirid]==$orderdir[$i][id_newsdir]) && $i!=0){//判断一级目录是否位置在最前，如果不到顶部就可以向上移动
			 
	             //             $orderdir[$i][id_newsdir] 当前ID      $orderdir[$i][ordernum]  当前顺序
				 //             $orderdir[$i-1][id_newsdir] 交换ID      $orderdir[$i-1][ordernum]  交换顺序
				 
				 $strSQL="UPDATE newsdir SET ordernum=". $orderdir[$i-1][ordernum]." WHERE id_newsdir=".$orderdir[$i][id_newsdir]."";
				 $objDB->Execute($strSQL);
				 $strSQL="UPDATE newsdir SET ordernum=".$orderdir[$i][ordernum]." WHERE id_newsdir=".$orderdir[$i-1][id_newsdir]."";	
				 $objDB->Execute($strSQL);
  ...
  if($_POST["arrow"]=='down'){//向下移动
	 ...
	 for($i=0;$i<$intnum;$i++){
		...
		if(($_POST[dirid]==$orderdir[$i][id_newsdir]) && $i!=$intnum-1){//判断一级目录是否位置在最后，如果不在最后就可以向下移动
