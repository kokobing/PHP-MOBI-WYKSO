...
ajax_upload_newsdir.php   分类目录 上传图片
...
if (!empty($_FILES) && $_POST['token'] == $verifyToken) {//如果上传不为空 并且令牌相等
	...
	if (in_array(strtolower($fileParts['extension']), $fileTypes)) {//扩展名正确匹配
		...
		//文件名入库
	     $strSQL="INSERT INTO newsdir_pic(id_newsdir,pic) values ('".$_POST[Tableid]."','".$new_file_name."')";
		...
		//回填HTML
		echo '<a href="'.$_POST['uploadPath'].$new_file_name.'" data-rel="prettyPhoto['.$_POST[Tableid].']" data-img_id="'.$pid.'" title="" ><img src="'.$_POST['uploadPath'].$new_file_name.'" width="35" height="35"></a>';
