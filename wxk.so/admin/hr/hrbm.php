<?php
require "../inc/core/hr/hrbm_core.php";
 ...
 <div class="vd_navbar vd_nav-width vd_navbar-chat vd_bg-black-80 vd_navbar-right   ">
  <?php require "../rightchat.php";?>	  
   ...
   <div class="row">
              <div class="col-md-12">
                <div class="panel widget">
                  <div class="panel-heading vd_bg-grey">
                    <h3 class="panel-title"> <span class="menu-icon">  <i class="fa fa-<?=$leftmenu_icon;?>"></i> </span>部门设置
                  ...
                  <div class="panel-body  table-responsive">
                    <table class="table table-bordered  table-hover" id="bmlist">
                      <thead>
                        <tr>
                          <th>#</th>
                          <th>部门名称 / 人数</th>
    ...
    $strSQL="SELECT bmname,hr_count,stat,indate FROM hr_bm ";//列表部门
	...
	$BmlistNum=sizeof($Bmlist);
					  ...
					  ?>
                        <tr>
                          <td><?=$i+1;?></td>
                          <td><?=$Bmlist[$i][bmname]?><span class="badge vd_bg-<? if($Bmlist[$i][hr_count]==0){echo 'red';}else{echo 'blue';}?>"><?=$Bmlist[$i][hr_count]?></span></td>
                          <td><?=$Bmlist[$i][indate]?></td>
                          <td><span class="label label-success">正常</span></td>
                          <td class="menu-action">
                      <? if($ispermit[ED]=='1'){?> <a  href="javascript:void(0)" onClick="editbm('<?=$Bmlist[$i][bmname]?>');" class="btn menu-icon vd_bg-yellow"> <i class="fa fa-pencil"></i> </a> <? }?>
                      <? if($ispermit[DE]=='1'){?> <a  href="javascript:void(0)" onClick="delbm('<?=$Bmlist[$i][bmname]?>','<?=$Bmlist[$i][hr_count]?>');" class="btn menu-icon vd_bg-yellow"> <i class="fa fa-times"></i> </a><? }?></td>
                        </tr>
 ...
 if($('<div/>').text(val).html()==''){
	   ...
	   notice.cancelRemove().update({
        ...
        buttons: {closer: true,sticker: false}
	 	...
	 	$.post('../ajax_php/hr/ajax_addbm.php',{bmname:$('<div/>').text(val).html()},function() {
           ...
           buttons: {closer: true,sticker: false}
			...
			$('<tr><td><span class="label label-danger">新增</span></td><td>'+$('<div/>').text(val).html()+'</td><td>刚刚</td><td><span class="label label-success">正常</span></td><td class="menu-action"><a  href="javascript:void(0)" onClick="editbm(\''+$('<div/>').text(val).html()+'\');" class="btn menu-icon vd_bg-yellow"> <i class="fa fa-pencil"></i> </a><a  href="javascript:void(0)" onClick="delbm(\''+$('<div/>').text(val).html()+'\',\'0\');" class="btn menu-icon vd_bg-yellow"> <i class="fa fa-times"></i> </a></td></tr>').insertBefore($('#bmlist tbody tr:eq(0)'));
...
(new PNotify({
    ...
    hide: false,
	buttons: {closer: true,sticker: false},
    confirm: {prompt: true,prompt_default:oldbmname, buttons: [{text: '确定',addClass: 'btn-default'},{text: '取消',addClass: 'btn-default'}]}
 ...
 if($('<div/>').text(val).html()==''){
	 ...
	 if(oldbmname!=$('<div/>').text(val).html()){ //旧的名称和新输入的名称不一致 才入库 则刷新本页 	
	 
	 	$.post('../ajax_php/hr/ajax_editbm.php',{newbmname:$('<div/>').text(val).html(),oldbmname:oldbmname},function() {
...
<? }?>
 ...
 <? if($ispermit[DE]=='1'){?>
//删除部门 弹屏
function delbm(oldbmname,bm_count){
            ...
            title: oldbmname+'部门删除提醒！',
             ...
             confirm: {confirm: true},
              buttons: {closer: false,sticker: false}
				...
				$.post('../ajax_php/hr/ajax_delbm.php',{oldbmname:oldbmname},function() {
