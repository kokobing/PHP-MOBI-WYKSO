<?php
require "../inc/core/news/internalnews_core.php";
...
<div class="row">
              <div class="col-md-12">
                <div class="panel widget light-widget panel-bd-top">
                  ...
                  <input id="search_keywords"  name="search_keywords"  type="text" class="input-sm" placeholder="搜索标题" style="width:35%;" value="<?=$_SESSION[internews_keywords];?>">
                  ...
                  <a class="btn vd_btn vd_bg-green btn-sm" href="/<?=SITE_DIR?>/news/internalnews-add.html">添加</a>
                    </div>
                      ...
                      <ul class="list-wrapper">
                       ...
                       <?php for($i=0;$i<$NewslistNum;$i++){?>
                        <li>
                          ...
                          <div class="menu-text " style="margin-left:10px; position:relative;">
                          
                           <div class="custom-btn-icon" style="position:absolute;right:10px;">
                           
                              <? if( $ispermit[ED]){?>
                              <a href="javascript:void(0)" onClick="setnewsarrow('<?=$Newslist[$i][id_newsinfo];?>','up')" data-original-title="<p style='width:50px;height:10px;'>向上移动</p>" data-html="true" data-toggle="tooltip" data-placement="top" class="btn custom-table-icon vd_bg-blue"><i class="fa fa-arrow-up"></i></a> 
                               <a href="javascript:void(0)" onClick="setnewsarrow('<?=$Newslist[$i][id_newsinfo];?>','down')" data-original-title="<p style='width:50px;height:10px;'>向下移动</p>" data-html="true" data-toggle="tooltip" data-placement="top" class="btn custom-table-icon vd_bg-blue"><i class="fa fa-arrow-down"></i></a>
							   <? }?>
                           
                           
       					   </div>
                          
                            <h3 class="blog-title font-bold letter-xs" style="width:70%;color:#000;"><a href="#"><?=$Newslist[$i][title];?></a></h3>
                            
                            <div class="menu-info"><div class="menu-date font-xs"><?=$Newslist[$i][newsdate];?></div></div>
                            
                            <?php if($Newslist[$i][tag]!=''){?>
                            <div class="menu-tags  mgbt-xs-15"> 标签: 
                            <?php 
							$arrtag=explode(",",$Newslist[$i][tag]);
							for($j=0;$j<sizeof($arrtag);$j++){?>
                            <a href="#"><span class="label vd_bg-grey"><?=$arrtag[$j];?></span></a> 
                            <?php }?>
                            </div>
                            <?php }?>

                            <div id="dirimgsbox<?=$i;?>"  class="menu-tags  mgbt-xs-15 newscallbackimgs" > 图片:
                              
                               <?
					    $strSQL="SELECT opicname as pic,id_newspic as pid,title FROM newspic WHERE  id_newsinfo='".$Newslist[$i][id_newsinfo]."' order by ordernum asc ";
						$objDB->Execute($strSQL);
						$OneNewsInfopics=$objDB->getrows();
						$OneNewsInfopicsnum=sizeof($OneNewsInfopics);
						for($j=0;$j<$OneNewsInfopicsnum;$j++){
								   ?>
                                      <a href="/upload/news/<?=$OneNewsInfopics[$j][pic]?>" data-rel="prettyPhoto[<?=$Newslist[$i][id_newsinfo];?>]" data-img_id="<?=$OneNewsInfopics[$j][pid]?>" title="<?=$OneNewsInfopics[$j][title]?>"> 
                                      <img src="/upload/news/<?=$OneNewsInfopics[$j][pic]?>" width="50" height="50">
                                      </a>
                                      
                                   <? }?>
                             </div>
                            
                            <p><?=$Newslist[$i][intro];?></p>
                            
                            
                            <div style="text-align:right">
                          
							  <? if($_SESSION[shenhe]=='1'){//有审核权?>
									 <? if($Newslist[$i][status]=='1'){ ?>
                                         <a href="javascript:void(0)" onClick="Mstatus('<?=$Newslist[$i][id_newsinfo]?>','<?=$Newslist[$i][status]?>');" class="btn vd_btn vd_bg-green btn-sm" style="float:left">已审核</a>
                                     <? }else{?>
                                         <a href="javascript:void(0)" onClick="Mstatus('<?=$Newslist[$i][id_newsinfo]?>','<?=$Newslist[$i][status]?>');" class="btn vd_btn vd_bg-red btn-sm" style="float:left">未审核</a>
                                     <? }?>   
                              <? }else{//无审核权?>
                                    <? if($Newslist[$i][status]=='1'){ ?>
                                        <span class="btn vd_btn vd_bg-red btn-sm" style="float:left">已审核</span>
                                    <? }else{?>
                                        <span class="btn vd_btn vd_bg-red btn-sm" style="float:left">未审核</span>
                                    <? }?>
                              <? }?>
                          ...
                          <!--组编功能-->
							<?php if($ispermit[GE]=='1'){?>
                                <a class="btn vd_btn vd_bg-green btn-sm" href="internalnewseditlist-<?=$Newslist[$i][id_newsinfo];?>.html">编辑</a> 
                          ...
                          <!--组删功能-->
                          <?php if($ispermit[GD]=='1'){?>
                          
                              <a href='javascript:void(0)' onclick='newsdel(<?=$Newslist[$i][id_newsinfo];?>);' class="btn vd_btn vd_bg-yellow btn-sm" >删除</a> 
                              
						  <?php }elseif($ispermit[GD]=='0'){?>
                          
                              <? if($ispermit[DE] && $Newslist[$i][id_hr]==$_SESSION[userid]){?>
                              <a href='javascript:void(0)' onclick='newsdel(<?=$Newslist[$i][id_newsinfo];?>);' class="btn vd_btn vd_bg-yellow btn-sm" >删除</a> 
							  <? }?>
						 ...
						 <?php }?>
                            ...
                            <!--上传图片进度条-->
                             <div id="progress<?=$i;?>" class="progress progress-striped" style="position:relative;bottom:-20px;background-color:#fff;" >
...
function searchkeywords(status){
	...
	$.post('../ajax_php/news/ajax_searchkeywords_internews.php',{search_keywords:$('#search_keywords').val(),status:status},function() {
             window.location.reload(); 
     });
