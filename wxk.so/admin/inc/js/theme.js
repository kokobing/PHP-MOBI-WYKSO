...
function pulltz(tz_title,tz_text,tz_icon,tz_type){
...
function fake_load(ctz_title,tz_title,tz_text,tz_icon,tz_type,is_reload) {
			var cur_value = 1,
				progress;
				...
				before_open: function(PNotify){
					...
					var timer = setInterval(function(){
						...
						if (cur_value >= 100) {
							...
							loader.remove();
							pulltz(tz_title,tz_text,tz_icon,tz_type);
							if(is_reload=='1'){setTimeout(function() {window.location.reload();  },1500); };
							return;
						}
						cur_value += 1;
						progress.width(cur_value+"%").attr("aria-valuenow", cur_value).find("span").html(cur_value+"%");
