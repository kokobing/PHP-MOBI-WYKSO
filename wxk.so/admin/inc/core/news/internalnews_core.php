...
//搜索存在
if(isset($_SESSION[internews_keywords])){
	$search_keywords=" and a.title like '%".$_SESSION[internews_keywords]."%' ";
}
...
$strSQLNum = "SELECT COUNT(*) as num FROM newsinfo as a
		   ...
		   where find_in_set('1',a.dirtree) $search_keywords" ;   
}else{
$strSQLNum = "SELECT COUNT(*) as num FROM newsinfo as a
           left join newsdir as b on a.id_newsdir=b.id_newsdir
		   left join hr as c on a.id_hr=c.id_hr
		   where a.id_hr='$_SESSION[userid]' and find_in_set('1',a.dirtree) $search_keywords";   
}
...
$objDB->Execute($strSQL);
 ...
 header('Location:internalnewseditlist-'.$_GET[edit].'.html'); exit();//跳转当前页
...
if($_GET[fun]=='add' && $ispermit[AD] ){
	...
	$strSQL="INSERT INTO newsinfo(id_newsdir,dirtree,title,intro,newsdate,id_hr) 
	        values('1','1','新增空白信息','新增空白信息简介',now(),'".$_SESSION[userid]."')";
      $objDB->Execute($strSQL);
   ...
   header('Location:internalnews.html'); exit();	//处理完毕跳转当前页	
}
