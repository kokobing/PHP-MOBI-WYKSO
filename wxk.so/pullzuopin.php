...
<div style="width:80%;margin:0 auto;">
 <form  id="formforbmset" action="#" method="post" >
          <div style="margin-top:15px;font-size:120%;font-weight:bold;">发布作品</div>
          <div style="margin-top:15px;"><input name="title" id="title"   type="text"  placeholder="标题" ></div>
          
         
         <div style="margin-top:15px;"><textarea name="intro" id="intro"  type="text" placeholder="简介"  ></textarea></div>
         
          
         
        <div style="margin-top:15px;">
        <select id="zptype"  name="zptype">
          <option value="0">选择作品类别</option>
          <option value="4">国画</option>
          <option value="5">油画</option>
        ...
        <div style="margin-top:15px;">
        <input id="zpdate"  name="zpdate" type="text" data-role="date" placeholder='创作日期 格式 ：<?=date('Y-m-d')?>'  >
        </div> 
         ...
         <div id="upbtnstyle" style="margin-top:15px;">
          ...
          <div style="margin-top:15px;"><a href="javascript:void(0)" onclick="nowuploadbtn();" class="ui-shadow ui-btn ui-btn-b ui-corner-all">发布作品</a></div>
...
<script type="text/javascript">
		...
		$(function() {
			...
			$('#file_upload-'+pagerandcode).uploadifive({
				...
				'uploadScript'     : '/ajax_php/pullzuopin_uploadifive.php',
		        'onUpload': function(filesToUpload) {},
				'onQueueComplete' : function(uploads) {

					  //$.mobile.changePage( "found.html" );
			
					  $.mobile.changePage("found.html", "slideup");
		...
		function nowuploadbtn(){
	     ...
	     if($('#title').val()!='' && $('#zptype').val()!='0' && $('#zpdate').val()!=''){	//not null
			$.post('/ajax_php/ajax_pullzuopin.php',{title:$('#title').val(),intro:$('#intro').val(),zptype:$('#zptype').val(),zpdate:$('#zpdate').val()},function(data) {
	        		 ...
	        		 $( "#MessagePopbox"+pagerandcode+" p" ).text( "需填写作品标题、类型、日期" );
