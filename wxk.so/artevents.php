...
$strSQL = "SELECT a.*,b.name as newsdirname,c.name as username,c.photo,c.id_member FROM eventinfo as a
		   ...
		   where  c.photo!='' and c.memid='".$_GET[fun]."'
...
//查找特定艺术家
$strSQL = "select id_member,name,intro,photo,level  from member where memid='".$_GET[fun]."' " ;
$objDB->Execute($strSQL);
$oneart=$objDB->fields();
...
<a href="artsinfo-<?=$_GET[fun]?>.html" class="ui-btn"><?=$oneart[name]?></a>
...
<div class="innercontent100">
   <div><?=$allNewsInfo[$i][title]?></div>
      ...
      <div class="innercontentimgs">
         <ul id="EventsGallery<?=$i?>" class="gallery">
